...
| **STT の実行場所** | AmiVoice（第一候補）/ その他 | **API の境界（音声を送る → テキストが返る）は不変**のため後から差し替えられる |
| **LLM の基盤** | Bedrock / 外部 API（Claude・GPT） | 同上。[#28 Bedrock連携基盤](https://github.com/study-basic-hackathon/hanasu/issues/28) との整合が要る |
...
| 確認相手 | 確認内容 | なぜ効くか |
|---|---|---|
| **チーム** | **外部の有償 API（AmiVoice / OpenAI 等）を使ってよいか。キーの配布・管理方法** | 使えない場合、**フィラー計測の実現方法から再検討**になる（Amazon Transcribe に `keepFillerToken` 相当の機能はない） |
| **TAKUO2000** | **[#29 Chime SDK基盤](https://github.com/study-basic-hackathon/hanasu/issues/29) を v1 不要としてよいか** | 着手済みなら手戻りになる。**早めの共有が要る** |
...
| **AmiVoice は商用の外部 API。** アカウント・課金・API キーの管理（ECS の環境変数 / Secrets Manager）の手当てが要る | 中 | ハッカソンで外部有償 API を使ってよいか。キーの配布方法 |
...
| **[#29 Chime SDK基盤](https://github.com/study-basic-hackathon/hanasu/issues/29) は D案では「v1 不要」と結論づけられている**（担当は TAKUO2000、着手済みの可能性あり） | 中 | チームでの確認。Issue を閉じるか、拡張シナリオ用に保留するか |
...
| **チーム** | **外部の有償 API（AmiVoice / OpenAI 等）を使ってよいか。キーの配布・管理方法** | 論点4・5・6 の選定がすべてここに依存する。使えない場合は Bedrock + Transcribe 系へ寄せ直しになり、**フィラー計測の実現方法を再検討する必要がある** |
