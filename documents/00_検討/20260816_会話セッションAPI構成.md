...
- ステータス: 完了
- **仕様の正本**: [API仕様.md](../backend/API仕様.md) — **現行の API 仕様はこちらを見る。** 本記録は決定に至る経緯であり、現行仕様ではない
- **本検討の結論は次の ADR に昇格済み**（2026-08-16、[#15](https://github.com/study-basic-hackathon/hanasu/issues/15)）
  - [ADR-0007 会話音声の取得と保存方式](../ADR/0007-会話音声の取得と保存方式.md) — 音声はブラウザで取得しターンごとに送信、永続化しない。Chime SDK / Realtime API を使わない
  - [ADR-0008 会話用APIの構成](../ADR/0008-会話用APIの構成.md) — API 7本、サーバーはステートレス、非同期評価
  - [ADR-0009 評価方式](../ADR/0009-評価方式.md) — 定量はクライアント算出、定性のみ LLM
...
| 12 | **文字起こし（STT）** | **AmiVoice を使う**（2026-08-16 確定）。`keepFillerToken=1` でフィラーが `%えー%` 形式で返る。**`raw_transcript` を絶対に捨てない。** フィラー抽出の精度は**実装して確かめてから判断する** |
| 13 | **会話・評価の LLM** | **Bedrock を使う**（2026-08-16 確定）。**アプリから呼ぶには ECS タスクロールの新設が必要**（現状は実行ロールのみ）であり、[#28 Bedrock連携基盤](https://github.com/study-basic-hackathon/hanasu/issues/28) が**会話機能のブロッカーになる** |
| 14 | **評価指標** | **①話の速さ ②フィラーの数 ③構成・内容**（+ **④間の長さは「できたら」**）。**「抑揚」「声の大きさ」は採らない** |
| 15 | **話速の算出** | **文字数 ÷ 発話時間。** 形態素解析（モーラ数）は使わない。**`/interviews/stt` が算出して返す** |
| 16 | **定量スコアの算出** | **フロントが算出**して `POST /evaluations` に渡す。**DB に保存するだけで LLM には渡さない** |
| 17 | **定性評価** | **LLM は `turns`（会話履歴）だけを見て評価する。** 数値は渡さない |
| 18 | **評価の実行方式** | **非同期。** `POST /evaluations` は `evaluation_id` だけを返し、`GET /evaluations/{id}` でポーリングして結果を取る |
| 19 | **評価の状態** | `processing` / `completed` / **`failed`**（失敗を表現しないと永久にポーリングし続ける） |
| 20 | **`/interviews/chat` の返し方** | **ストリーミングしない。`{ "text": "..." }` を1回の JSON で返す。** SSE は実装しない |
| 21 | **TTS** | **必須ではない。余裕があれば作る。使用するサービスは未定**（2026-08-16）。実装する場合も**発言全体を1回で音声化する**。**「句読点が来たタイミングで TTS に投げる」方式は実装しない。** 作らない場合は面接官の発言を文字表示するだけでよく、**他の4本はそのまま成立する** |
| 22 | **DB** | **応募情報テーブル / 評価結果テーブル**の2つ（+ 実装済みの `users`）。**会話履歴・音声・transcript は保存しない** |
| 23 | **評価結果の PK** | **`evaluation_id` 単体（サロゲートキー）。** `company_id` は FK + INDEX。**`session_id` はサーバー側では持たない**（`evaluation_id` と 1:1 で重複するため） |
| 24 | **Chime SDK** | **v1 では使わない。** 1対1かつ相手が AI のため代替不能な機能がなく、面接官 AI をアテンディーとして参加させるにはヘッドレスクライアントの常駐が必要になる |
| 25 | **Realtime API** | **v1 では使わない。** 音声はテキストの約10倍のトークンを消費し、32,768 トークン窓では15〜20分で上限に達しうる |
...
| **Bedrock のモデル選定** | 会話用・評価用にどのモデルを使うか | **基盤は Bedrock で確定**（決定事項13）。モデルの選定と、`backend/pyproject.toml` への `boto3` 追加は実装時 |
| **AmiVoice の API キー管理** | ECS の環境変数 / Secrets Manager | 既存実装は `JWT_SECRET_KEY` を環境変数で受けている（`backend/app/security.py`）ため、**同じ形に揃えるのが素直** |
| **TTS のサービス選定** | **未定**（2026-08-16）。Amazon Polly / OpenAI TTS / VOICEVOX 等 | **TTS 自体が任意**のため、作ると決めた時点で選べばよい。**Bedrock 採用で ECS タスクロールが新設されるため、Polly なら `polly:SynthesizeSpeech` を足すだけで済む**（外部 API はキー管理が増え、VOICEVOX は自前ホストが要る） |
...
| 確認相手 | 確認内容 | なぜ効くか |
|---|---|---|
| **TAKUO2000** | **[#28 Bedrock連携基盤](https://github.com/study-basic-hackathon/hanasu/issues/28) に ECS タスクロールの新設が含まれるか。着手状況と完了見込み** | **LLM は Bedrock で確定したため、[#28](https://github.com/study-basic-hackathon/hanasu/issues/28) は会話機能のブロッカーになった。** 現状の実行ロールは ECR pull とログ出力用で、アプリからの `bedrock:InvokeModel` には使えない |
...
- **[必要api .md](../必要api.md)** — 「会話用API: TODO：調べる」を本結論で埋める。また「音声分析: 抑揚・フィーラー・声の大きさ・スピード」は**抑揚と声の大きさが落ちる**ため実態と合わなくなる（[#34](https://github.com/study-basic-hackathon/hanasu/issues/34) の完了条件）
...
会話セッションの開始から終了までに必要な API が未定義で、[必要api .md](../必要api.md) の「会話用API」は `TODO：調べる` のまま。会話画面（[#27](https://github.com/study-basic-hackathon/hanasu/issues/27) / [#9](https://github.com/study-basic-hackathon/hanasu/issues/9)）もバックエンド実装も着手できない。
...
| 4 | **文字起こしの実行場所** — ECS 上のセルフホスト（Whisper）/ マネージド（Transcribe 等）/ 外部 API | 外部 API（AmiVoice が第一候補）— フィラー計測が決め手 | **✅ AmiVoice で確定**（2026-08-16。外部 API の利用可否をチーム確認済み） |
| 5 | **AI 応答の生成基盤** — Bedrock / 外部 LLM API | Claude / GPT の chat API | **✅ Bedrock で確定**（2026-08-16）。**[#28](https://github.com/study-basic-hackathon/hanasu/issues/28) が会話機能のブロッカーになる** |
...
| 23 | **（新規）評価指標を何にするか** — 出典によって食い違っている。**論点15（音響メタの粒度）はこれに従属する** | 話速・文章構成・話の内容・フィラー数（メモ1章）。データモデルには「間」も含む（メモ4.5） | **✅ 確定**（2026-08-16）— 必須3つ **①話の速さ ②フィラーの数 ③構成・内容** + **④間の長さ（優先度「できたら」）**。**「抑揚」「声の大きさ」は採らない**（[必要api .md](../必要api.md) の更新は [#34](https://github.com/study-basic-hackathon/hanasu/issues/34) で行う） |
...
| 25 | **（新規）会話系 API に認証を課すか** | — | **✅ JWT Bearer を課す**（2026-08-16）。[必要api .md](../必要api.md) の制約を満たす |
...
すべて `Depends(get_current_user)` で JWT を検証する（[必要api .md](../必要api.md) の「認証ユーザーのみ実行可能」制約）。
...
| 2 | **認証** | 全 API に **JWT Bearer**（実装済みの `Depends(get_current_user)`） | **対象外**（「ハッカソンでは不要」） | [必要api .md](../必要api.md) の「**認証ユーザーのみ実行可能**」制約と衝突 |
...
| **認証制約の遵守** | [必要api .md](../必要api.md) の「認証ユーザーのみ実行可能」を満たす |
...
| 2 | **認証** | **E案** | **JWT Bearer**（実装済みの `Depends(get_current_user)`）。[必要api .md](../必要api.md) の「認証ユーザーのみ実行可能」制約を満たす |
...
| **認証なしの方針が [必要api .md](../必要api.md) の「認証ユーザーのみ実行可能」と衝突する** | 中 | 制約を維持するか、ハッカソンでは外すか |
...
- **[必要api .md](../必要api.md) の「音声分析: 抑揚・フィーラー・声の大きさ・スピード」は実態と合わなくなるため、更新が必要**（[#34](https://github.com/study-basic-hackathon/hanasu/issues/34) の完了条件に含まれる）
...
- [必要api .md](../必要api.md) の更新は **[#34](https://github.com/study-basic-hackathon/hanasu/issues/34) の作業として後で行う**（本ラウンドでは触らない）
...
### 2026-08-16 第17ラウンド（STT と LLM の基盤が確定）

- **ユーザー判断: 会話・評価の LLM は Bedrock、STT は AmiVoice を使う**
- これにより **1.4 で「実装時検討」としていた論点4（STT の実行場所）・論点5（LLM の基盤）が確定**した
- **重要な帰結: [#28 Bedrock連携基盤](https://github.com/study-basic-hackathon/hanasu/issues/28) が会話機能のブロッカーになった**
  - アプリから Bedrock を呼ぶには **ECS タスクロールの新設と `bedrock:InvokeModel` の許可が必要**（`infra/dev/ecs.tf` / `infra/dev/iam.tf`）
  - 現状の実行ロール（`ecs_task_execution`）は **ECR pull とログ出力用**であり、アプリ内からの AWS API 呼び出しには使えない
  - 従来は「Bedrock / 外部 API のどちらでも成立する」状態だったため [#28](https://github.com/study-basic-hackathon/hanasu/issues/28) は必須ではなかったが、**Bedrock 確定により依存が発生した**
- 実装時に残る作業として、**Bedrock のモデル選定**・**`backend/pyproject.toml` への `boto3` 追加**・**AmiVoice の API キー管理**（既存の `JWT_SECRET_KEY` と同じく環境変数で受けるのが素直）を 1.4 に記載した
- **評価用の LLM も Bedrock を使う**ことをユーザーに確認済み（2026-08-16）。会話・評価の両方が同じ基盤になる
- **TTS で使用するサービスは未定**（2026-08-16 ユーザー）。TTS 自体が任意のため、作ると決めた時点で選べばよい
  - **Bedrock 採用により ECS タスクロールが新設されるため、Amazon Polly なら `polly:SynthesizeSpeech` を足すだけで済む**。外部 API（OpenAI TTS）はキー管理が増え、VOICEVOX は自前ホストが要る
