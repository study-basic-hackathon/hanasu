...
- ステータス: 完了
- **昇格先**: [ADR-0017 GitHub Actions による Amplify 直接配備と CloudFront 経由の API 公開](../ADR/0017-GitHubActionsによるAmplify直接配備とCloudFront経由のAPI公開.md) — 決定内容はこちらが正本
...
今回の方針は、Amplify を Git リポジトリに接続せず、GitHub Actions がビルドした静的成果物を Amplify の直接配備 API で配備する方式である。GitHub Actions は Terraform の `apply` / `destroy` には使わない。公開環境の API は、CloudFront を現在の internet-facing ALB の前段に置き、CloudFront の HTTPS URL 経由で接続する。Cognito は利用しないため、Cognito 関連の Amplify 環境変数・リソースは対象外とする。

## 論点

- 必須: Amplify ネイティブの Git ブランチデプロイと、GitHub Actions による直接配備のどちらを採用するか。
...
- 任意: 地理的制限、ZIP 作成・アップロード workflow、origin header のローテーションの具体的な運用手順。
...
- Amplify は Git プロバイダに接続せず、`CreateDeployment` が返す URL へ build output の ZIP をアップロードして直接配備できる。Git 非接続の直接配備は SSR をサポートしない。ZIP は build output の最上位フォルダではなく、その**中身**をルートに含める必要がある。出典: [AWS Amplify の Git 非接続デプロイ](https://docs.aws.amazon.com/amplify/latest/userguide/manual-deploys.html)。
...
| 観点 | A案: Amplify ネイティブ Git ブランチデプロイ | B案: GitHub Actions からの直接配備 |
|---|---|---|
| ビルド主体 | Amplify | GitHub Actions |
| GitHub 連携 | 必要。GitHub App とトークンを扱う | 不要。App に `repository` を設定しない |
| 秘密情報の扱い | 連携用トークンを安全な注入経路で一時的に渡す必要がある | GitHub OIDC を使い、長期 AWS アクセスキーを置かない |
| IaC 管理 | App / branch に加え Git 連携設定の扱いを決める必要がある | App / branch と配備用 IAM ロールを Terraform 管理。workflow はリポジトリで管理 |
| ビルド設定 | `amplify.yml` または Amplify コンソール | GitHub Actions で `NEXT_PUBLIC_API_BASE_URL` を指定して `next build`。`amplify.yml` は不要 |
| API URL のビルド時注入 | Amplify App/branch の環境変数から渡す | GitHub Actions のビルド時環境変数として渡す |
| 今回との整合 | GitHub 接続を持たない方針と合わない | GitHub 接続なしで CI 配備でき、Terraform の適用権限とも分離できる |
...
| Amplify の環境変数を外部ビルドでも使えると誤解する | 高 | B案では GitHub Actions のビルド時に `NEXT_PUBLIC_API_BASE_URL` を注入する |
| 提示 IaC の `AdministratorAccess-Amplify` をそのまま付与する | 中 | 静的な直接配備に Amplify のサービスロールは不要とし、作成しない |
...
|---|---|---|
| チーム | B案（GitHub Actions からの直接配備）を正式採用するか | #19 / #92 の実装前提と ADR の結論になる |
...
- 2026-08-22: ユーザーから、GitHub Actions を使わない対象は Terraform の `apply` であり、フロントエンドの Amplify 配備には GitHub Actions を使うとの訂正を受領。ADR-0016 を ADR-0017 で置き換え、Git 非接続 App に対する `CreateDeployment` / ZIP upload / `StartDeployment` の直接配備へ変更した。

## 結論

次を決定した。

1. Amplify App は Git 非接続の静的ホスティングとして Terraform で作成し、GitHub Actions がビルドした `out/` の中身を ZIP にして Amplify の直接配備 API へ送る。GitHub Actions は Terraform の `apply` / `destroy` を実行しない。GitHub 連携、`amplify.yml`、Cognito、Amplify サービスロール、`AdministratorAccess-Amplify` は作成しない。
...
5. フロントエンドは `NEXT_PUBLIC_API_BASE_URL=https://<CloudFront domain>` を GitHub Actions のビルド時に与える。CORS は Amplify branch domain を ECS の `CORS_ALLOWED_ORIGINS` に渡して許可する。
