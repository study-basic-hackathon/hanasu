...
export const QA_MAX_SECONDS = 60;

type QaPracticeSessionProps = {
  question: string;
};
...
function speakingSpeedComment(charsPerMinute: number): string {
  ...
  if (charsPerMinute > SPEAKING_SPEED_RANGE.max) {
    return "少し速めです。句読点で間を取り、落ち着いて話しましょう。";
  }
...
/** S-13 の音声回答と、保存しない話速・フィラー計測UI。 */
export function QaPracticeSession({ question }: QaPracticeSessionProps) {
  const recording = usePracticeRecording({ maxSeconds: QA_MAX_SECONDS });
  const isBusy =
    recording.phase === "requesting" || recording.phase === "transcribing";
  const progress = Math.min(
    100,
    (recording.elapsedSeconds / QA_MAX_SECONDS) * 100,
  );
  ...
  return (
    <>
      <Card className="flex flex-col gap-2 p-6">
        ...
        <p className="text-[17px] leading-[1.7] font-medium">{question}</p>
      </Card>

      {recording.phase === "success" && recording.result ? (
        <section aria-labelledby="qa-result-title" className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h2 id="qa-result-title" className="text-card font-bold">
              今回の評価
            </h2>
