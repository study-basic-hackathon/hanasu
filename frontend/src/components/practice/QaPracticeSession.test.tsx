...
describe("QaPracticeSession", () => {
  ...
  function renderSession() {
    return render(<QaPracticeSession question="なぜ当社を志望されたのですか。" />);
  }

  async function startRecording() {
    await act(async () => {
      fireEvent.click(screen.getByRole("button", { name: "回答の録音を開始する" }));
    });
  }
  ...
  it("音声回答を文字起こしし、話速とフィラーだけを一時表示する", async () => {
    mocks.transcribeAudio.mockResolvedValue(transcription);
    renderSession();

    expect(screen.getByText("なぜ当社を志望されたのですか。")).toBeInTheDocument();
    ...
    expect(screen.queryByText(/内容|関連性|文章構成/)).not.toBeInTheDocument();

    fireEvent.click(screen.getByRole("button", { name: "もう一度回答する" }));
    expect(screen.getByRole("button", { name: "回答の録音を開始する" })).toBeEnabled();
  });

  it("文字起こしに失敗しても同じ音声を再送できる", async () => {
